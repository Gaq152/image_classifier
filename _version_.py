...
VERSION_HISTORY = [
    ...
    },
    {
        ...
        "details": [
            "【搜索与筛选功能 - 新增】",
            "新增：ExpandableSearch 可展开搜索组件，支持动画展开/收起",
            "新增：类别列表搜索功能，按回车确认搜索，点击X或收起时恢复列表",
            "新增：类别按数量排序筛选项",
            "新增：升序/降序切换按钮，动态展示排序方向",
            "修复：搜索清空后列表不恢复的问题（_is_active_search 状态追踪）",
            "",
            "【图片列表性能优化 - 重大改进】",
            "新增：ImageListModel 数据模型，分离数据与视图",
            ...
            "新增：智能缩放策略（S0/S1/S2），根据文件大小自动选择缩放方案",
            ...
            "修复：重复连接信号导致的日志重复打印问题（UniqueConnection）",
    ...
    },
    {
        "version": "6.5.0",
        "date": "2025-11-08",
        "title": "设置系统重构与用户体验全面升级",
        "highlights": [
            "⚙️ 全新设置页面：独立设置界面，所有配置项集中管理，支持实时预览",
            "🎨 主题系统增强：支持跟随系统主题、自动切换、与主窗口实时同步",
            "📊 日志与提示配置：可自定义日志级别和 Toast 提示级别，精准控制信息显示",
            "🎓 教程系统扩展：从 7 步扩展到 18 步，新增设置页面引导和虚拟菜单演示",
            "🔄 上次任务恢复：记录并恢复上次处理位置，工作流更连续",
            "🔍 缩放功能增强：支持全局缩放配置、可限制最大/最小缩放倍数（最高 20 倍）",
            "🏷️ 分类状态标记：可视化显示图片分类状态，一目了然",
            "📚 帮助文档重构：全面更新快速入门、使用指南、FAQ 和关于页面",
            "🎯 样式统一优化：多处 UI 组件样式优化，视觉体验更一致"
        ],
        "details": [
            "【设置页面系统 - 重大新增】",
            "新增：独立的设置对话框，所有配置项统一管理",
            "新增：设置项分类展示（快捷键、缩放、主题、日志、更新等）",
            "新增：全局配置文件 app_config.json，与目录配置分离",
            "新增：设置输入框编辑功能美化，支持实时预览",
            "新增：设置页面主题切换实时生效，与主窗口图标同步",
            "优化：设置面板样式统一，调整设置项顺序更合理",
            "优化：配置文件架构重构，全局配置生效机制",
            "",
            "【主题系统 - 功能增强】",
            "新增：跟随系统主题功能，自动检测系统主题变化",
            "新增：主题自动切换配置，支持定时切换",
            "修复：主题同步问题，确保所有组件颜色一致",
            "修复：主题切换加载问题，避免初始化时闪烁",
            "改进：设置页面主题切换实时生效，无需重启",
            "改进：主窗口图标与主题完全同步",
            "",
            "【日志和提示系统 - 用户可配置】",
            "新增：日志级别配置（DEBUG/INFO/WARNING/ERROR）",
            "新增：Toast 提示级别配置，控制弹窗提示的详细程度",
            "重构：日志记录级别系统，区分不同等级",
            "优化：去除冗余日志打印，提升性能",
            "增强：更新对话框功能，显示更详细的更新信息",
            "修复：部分日志显示级别错误问题",
            "改进：样式统一，日志和提示界面更一致",
            "",
            "【教程系统 - 全面扩展】",
            "扩展：教程从 7 步增加到 18 步，覆盖更多功能",
            "新增：设置页面教程引导，帮助用户了解配置选项",
            "新增：虚拟菜单渲染，教程中可交互演示",
            "修复：教程文本错误，描述更准确",
            "修复：教程和恢复任务同时出现的冲突问题",
            "改进：教程步骤更详细，新手体验更友好",
            ...
            "新增：记录上次处理的图片位置",
            "新增：启动时提示是否恢复上次位置",
            "新增：jump_to_image() 方法支持快速跳转",
            "修复：教程和恢复提示同时出现的问题",
            "改进：工作流连续性，减少重复操作",
            "",
            "【缩放功能 - 重大增强】",
            "新增：全局缩放配置功能",
            "新增：支持限制最大缩放倍数（最高 20 倍）",
            "新增：支持限制最小缩放倍数（最低 0.01 倍）",
            "新增：缩放设置复用全局配置",
            "改进：缩放体验更灵活，满足不同使用场景",
            "",
            "【分类状态标记 - 新增功能】",
            "新增：图片分类状态可视化标记",
            "新增：状态标记显示在图片列表中",
            "改进：一眼识别已分类、未分类、已移除状态",
            "",
            "【帮助文档 - 全面重构】",
            "快速入门：补充 WebP 格式，扩展使用技巧到 12 项",
            "使用指南：修正缩放倍数、按钮描述、撤销操作等内容",
            "使用指南：扩展高级设置到 3 个子部分共 14 项",
            "FAQ 页面：删除 6 个冗余问题，新增教程和更新相关问题",
            "关于页面：版本历史精简到最近 3 个版本",
            "关于页面：新增动态 GitLab CHANGELOG 链接",
            "删除：高级功能标签页（内容重复且过时）",
            "修复：所有已知的不准确、过时和错误描述",
            "",
            "【GitLab CI/CD - 动态分支支持】",
            "新增：release 任务动态检测 tag 所在分支",
            "新增：使用 git branch -r --contains 获取分支名",
            "新增：通过 dotenv artifacts 传递分支名变量",
            "修复：CHANGELOG 链接从硬编码 main 改为动态 $CHANGELOG_BRANCH",
            "改进：支持从任意分支推送 tag 时正确链接到对应分支的 CHANGELOG",
            "",
            "【UI 样式优化】",
            "修复：添加类别按钮样式问题",
            "修复：多处按钮样式不一致问题",
    ...
    },
    {
        ...
        "highlights": [
            ...
            "🔧 配置系统重构：分离全局配置与目录配置，架构更清晰合理"
        ],
        "details": [
            "【教程系统 - 重大新增】",
            "新增：TutorialManager 教程管理器，控制七步引导流程",
            "新增：TutorialBubble 气泡组件，支持智能定位和边界检测",
            "新增：TutorialOverlay 遮罩层，聚焦当前教程目标组件",
            "新增：MockCategoryButton / MockImageListItem 虚拟组件系统",
            "新增：七步教程流程：欢迎 → 添加类别 → 图片列表 → 快捷键 → 统计面板 → 模式切换 → 完成",
            "新增：教程完成/跳过状态记录到 app_config.json",
            "新增：首次启动自动触发教程，已完成用户不再显示",
            "新增：教程气泡支持上/下/左/右四个方向自适应定位",
            "新增：教程气泡三角箭头动态调整，指向目标组件",
            "新增：教程界面完全响应主题切换，颜色和样式自动适配",
            "",
            "【配置管理 - 架构优化】",
            "新增：全局 app_config.json 管理应用级配置（主题、教程、更新设置）",
            "新增：配置自动迁移功能，从旧的 config.json 迁移更新配置到全局文件",
            "优化：utils/app_config.py 增加更新配置的属性（auto_update_enabled 等）",
            "优化：core/config.py 移除重复的 AppConfig 类，使用统一的全局配置",
            "改进：配置文件结构更合理，图片目录相关配置和全局配置分离",
            "",
            "【用户体验 - 功能增强】",
            "新增：上次位置恢复功能，启动时提示是否跳转到上次处理的图片",
            "新增：jump_to_image() 方法支持跳转到指定索引的图片",
            "新增：_check_and_prompt_last_position() 检查并提示用户恢复位置",
            "修复：init_ui() 中添加 apply_theme() 调用，确保主题完全加载",
            "修复：apply_theme() 中错误调用 category_buttons.values() 的问题",
            "修复：AddCategoriesDialog 添加居中显示逻辑和 showEvent 处理",
            "修复：跳转位置对话框支持主题颜色，暗主题下清晰可读",
            "优化：状态文件 last_index 加载和保存逻辑完善"
        ]
    },
    {
        "version": "6.3.3",
        ...
        "title": "路径统一管理与更新体验优化",
        "highlights": [
            "📁 统一路径管理：cache、logs、update统一到C:\\Users\\<用户名>\\image_classifier",
            "🗑️ 日志自动清理：7天轮转机制，启动时自动清理旧日志",
            "🔄 更新检查优化：频率改为1小时，增加HTTP 502重试机制",
            "🔴 红点提醒功能：更新可用时在按钮上显示醒目红点标记",
            "⚡ 下载体验提升：SHA256校验不再卡死UI，显示校验进度"
        ],
        "details": [
            "新增：utils/paths.py 统一路径管理模块",
            "新增：get_app_data_dir() 返回 C:\\Users\\<用户名>\\image_classifier",
            "新增：get_cache_dir() / get_logs_dir() / get_update_dir() 路径函数",
            "新增：get_old_cache_dir() 用于旧路径迁移",
            "新增：日志7天轮转机制，使用TimedRotatingFileHandler",
            "新增：启动时自动清理7天前的日志文件",
            "新增：BadgeWidget 组件，支持按钮右上角红点标记",
            "新增：帮助按钮和检查更新按钮的红点提醒功能",
            "新增：HTTP 502/503/504 错误3次重试机制，指数退避（0.5s/1s/2s）",
            "新增：sha256_file() 支持processEvents()，避免UI卡死",
            "新增：下载对话框显示'正在校验文件完整性...'提示",
            "优化：更新检查间隔从6小时改为1小时",
            "优化：红点显示逻辑，用户取消更新时持续显示提醒",
            "优化：批处理脚本路径使用统一的update目录",
            "优化：SHA256校验每5MB处理一次UI事件，防止长时间无响应",
            "修复：红点在特定交互场景下不显示的问题",
            "修复：launch_self_update 重复导入导致的引用错误",
            "修复：下载完成后到弹出确认对话框的18秒延迟问题",
            "改进：所有应用数据集中在可见目录，方便用户管理和清理"
        ]
    },
    {
        "version": "6.3.2",
        ...
        "title": "菜单系统UI优化与智能定位",
        "highlights": [
            "✨ 新增排序下拉按钮：类别标题栏添加排序按钮(▼)，与筛选按钮对称",
            "🎨 自定义复选框图标：18×18精美复选框替代Qt默认样式",
            "🔧 智能菜单定位：防止菜单超出窗口边界，自动调整位置",
            "🖼️ emoji图标一致性：使用⚠️✅❌图标与图片列表视觉统一",
            "⚡ 优化菜单布局：移除重复排序选项，精简筛选提示信息"
        ],
        "details": [
            "新增：类别标题栏排序下拉按钮(▼)，橙色透明背景，与筛选按钮对称",
            "新增：create_checkbox_icon()方法绘制18×18自定义复选框图标",
            "新增：智能菜单定位算法，使用menu.adjustSize()获取准确尺寸",
            "新增：菜单右对齐显示，使用bottomRight()替代bottomLeft()",
            "新增：窗口边界检测，使用self.rect()和mapToGlobal()计算边界",
            "优化：菜单图标尺寸从72×40px缩小为20×20px，改善文字间距",
            "优化：筛选菜单使用emoji图标(⚠️未分类 ✅已分类 ❌已移除)",
            "移除：类别按钮右键菜单的重复排序选项(📝按名称排序/⌨️按快捷键排序)",
            "移除：筛选菜单底部的'✓ 已应用筛选: 显示 X/Y 张'提示",
            "修复：菜单超出窗口右边界和底部边界的问题",
            "修复：菜单图标间距过大导致布局异常的问题",
            "改进：所有菜单统一使用自定义复选框图标，视觉效果一致"
        ]
    },
    {
        "version": "6.3.1",
