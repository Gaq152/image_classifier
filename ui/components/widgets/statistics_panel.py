...
class RoundedProgressBar(QProgressBar):
    ...
    def __init__(self, parent=None):
        ...
        self._chunk_colors = (
            ...
            default_theme.colors.SUCCESS_DARK,
        )
        # 清空默认样式，完全由自绘控制
        self.setStyleSheet("""
    ...
    def setChunkColors(self, start_color: str, end_color: str):
        """设置进度块的主题化渐变色。"""
        self._chunk_colors = (start_color, end_color)
        self.update()
    ...
    def paintEvent(self, event):
        ...
        if progress > 0:
            ...
            gradient.setColorAt(0, QColor(self._chunk_colors[0]))
            gradient.setColorAt(1, QColor(self._chunk_colors[1]))
...
class StatisticsPanel(QWidget):
    ...
    def update_statistics(self, total, classified, removed, display_count=None):
        ...
        try:
            ...
            if total > 0:
                ...
                self.progress_bar.setChunkColors(
                    *self._progress_colors(progress_percentage)
                )
            else:
                self.progress_bar.setValue(0)
                self.progress_bar.setChunkColors(
                    default_theme.colors.ERROR,
                    default_theme.colors.ERROR_DARK,
                )
    ...
    @staticmethod
    def _progress_colors(value: int):
        """按进度返回设计系统中的语义色。"""
        c = default_theme.colors
        if value >= 100:
            return c.SUCCESS, c.SUCCESS_DARK
        if value >= 75:
            return c.PRIMARY, c.SUCCESS
        if value >= 50:
            return c.WARNING, c.WARNING_DARK
        return c.ERROR, c.ERROR_DARK
    ...
    def apply_theme(self):
        ...
        try:
            ...
            if hasattr(self, 'progress_bar'):
                ...
                self.progress_bar.setChunkColors(
                    *self._progress_colors(current_value)
                )
